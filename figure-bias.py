...
data_dir = "shan"
bcsd = xr.open_dataset(os.path.join(data_dir,'monthly-bias_bcsd-obs.nc')).pr.mean(dim='time')
gcm = xr.open_dataset(os.path.join(data_dir,'monthly-bias_gcm-obs.nc')).PRECTOTLAND.mean(dim='time')  #.salem.roi(shape=chnshp)
#NEX = xr.open_dataset(os.path.join(data_dir,'monthly-bias_NEX-obs.nc')).pr.mean(dim='time')  #.salem.roi(shape=chnshp)
...
#newmap = cmaps.BlWhRe
newmap = cmaps.MPL_BrBG   

# pplt.rc.fontname = 'DejaVu Sans'
rrticks = np.arange(-60, 70, 10)
f, axs = pplt.subplots(proj=Alberts_China, nrows=1, ncols=2, share=True)
...
#    ax_inset.hist(data[i].values.flatten(), bins=10, density=False, alpha=0.7, lw=0.5, color='lightgray')
...
f.save('111-Fig3-2-bias_pr-monthly.jpg', transparent=True, dpi=600, pad_inches = 0.05, bbox_inches='tight')
